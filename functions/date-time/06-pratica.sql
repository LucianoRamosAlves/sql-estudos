...
/*
   Mesmo resultado do BETWEEN, mas usando a função YEAR()
*/
...
/*
   ORDER BY com datas:
   ASC  (padrão): mais antigas primeiro
...
-- ========================================================================
-- 4. CALCULANDO IDADE COM TIMESTAMPDIFF
-- ========================================================================
...
/*
   Resultado (exemplo para outubro):
...
/*
   ...
   - A data de aniversário deste ano já passou
   
   Lógica: monta a data de aniversário deste ano
   (ano atual + mês/dia do nascimento) e compara com hoje
*/
...
/*
   ...
   Maria Lima     | 1988-12-25 | 2024-12-25   ← ainda não
...
-- ========================================================================
-- 7. DESAFIO: Dias para o próximo aniversário
-- ========================================================================
...
SELECT
    ...
    nascimento,
    CASE
        WHEN CONCAT(YEAR(CURDATE()), '-', MONTH(nascimento), '-', DAY(nascimento))
             >= CURDATE()
        THEN DATEDIFF(
            CONCAT(YEAR(CURDATE()), '-', MONTH(nascimento), '-', DAY(nascimento)),
            CURDATE()
        ...
        ELSE DATEDIFF(
            CONCAT(YEAR(CURDATE()) + 1, '-', MONTH(nascimento), '-', DAY(nascimento)),
            CURDATE()
        )
    END AS dias_proximo_aniversario
FROM pessoas
ORDER BY dias_proximo_aniversario;

/*
   Resultado (se for 05/10/2024):
   
   nome           | nascimento | dias_proximo_aniversario
   ---------------+------------+--------------------------
   Carlos Souza   | 2000-10-05 | 0          ← hoje!
   João Santos    | 2000-10-20 | 15         ← 15 dias
   Ana Silva      | 1995-06-15 | 253        ← próximo ano
   Maria Lima     | 1988-12-25 | 81         ← 81 dias
*/
